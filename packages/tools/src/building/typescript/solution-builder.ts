...
interface PackageBuildContext {
  fileWrite?: (
    writeFile: typeof sys.writeFile,
    filename: string,
    contents: string,
    writeBom?: boolean,
  ) => void;
...
function instrumentHostWithInterceptors(
  ...
  const moduleIdentifier = graph
    ? new RegExp(
        `^(${graph.nodes.map((node) => node.pkg.path).join("|")})[\\/]dist[\\/](?:esm|cjs)/(.*).d.ts$`,
      )
    : undefined;
  ...
  const resolveModuleNameLiteralsInterceptor: CompilerHost["resolveModuleNameLiterals"] = (
    ...args
  ) => {
