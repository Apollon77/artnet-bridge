...
export declare interface ArtNetReceiver {
    on<K extends keyof ArtNetReceiverEvents>(event: K, listener: (...args: ArtNetReceiverEvents[K]) => void): this;
    emit<K extends keyof ArtNetReceiverEvents>(event: K, ...args: ArtNetReceiverEvents[K]): boolean;
    off<K extends keyof ArtNetReceiverEvents>(event: K, listener: (...args: ArtNetReceiverEvents[K]) => void): this;
    once<K extends keyof ArtNetReceiverEvents>(event: K, listener: (...args: ArtNetReceiverEvents[K]) => void): this;
}
...
export class ArtNetReceiver extends EventEmitter {
    ...
    async start(): Promise<void> {
        ...
        socket.on("message", (msg, rinfo) => {
            const packet = parsePacket(msg as Buffer);
            if (!packet) return;
