...
export function validateMappings(mappings: DmxChannelMapping[]): string[] {
  ...
  for (const mapping of mappings) {
    ...
    if (mapping.dmxStart < 1 || mapping.dmxStart > 512) {
      errors.push(
        `Mapping for ${mapping.targetId}: dmxStart ${mapping.dmxStart} out of range (1-512)`,
      );
    }
    if (end > 512) {
      errors.push(
        `Mapping for ${mapping.targetId}: channels exceed universe boundary (dmxStart=${mapping.dmxStart}, end=${end})`,
      );
    }
  }
  ...
  for (let i = 0; i < sorted.length - 1; i++) {
    ...
    if (currentEnd >= next.dmxStart) {
      errors.push(
        `Mapping overlap: ${current.targetId} (${current.dmxStart}-${currentEnd}) overlaps with ${next.targetId} (${next.dmxStart})`,
      );
    }
  }
